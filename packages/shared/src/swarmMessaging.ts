...
const DIRECTIVE_ALLOWED_PRECEDING_CHAR_REGEX = /[.!?:;)\]}]/;
...
function isInsideInlineCodeSpan(text: string, index: number): boolean {
  const lineStart = text.lastIndexOf("\n", index - 1) + 1;
  const beforeOnLine = text.slice(lineStart, index);
  const tickCount = beforeOnLine.match(/`/g)?.length ?? 0;
  return tickCount % 2 === 1;
}

function isDirectiveContextAllowed(text: string, index: number): boolean {
  if (isInsideInlineCodeSpan(text, index)) {
    return false;
  }

  const lineStart = text.lastIndexOf("\n", index - 1) + 1;
  const beforeOnLine = text.slice(lineStart, index);
  if (/^\s*(?:[-*+]\s+|>\s*)?$/.test(beforeOnLine)) {
    return true;
  }
  ...
  return DIRECTIVE_ALLOWED_PRECEDING_CHAR_REGEX.test(text[cursor] ?? "");
}
...
function findNextSwarmControl(text: string, fromIndex: number): SwarmControlMatch | null {
  ...
  const directiveMatches = [
    ...
    .map((regex) => {
      ...
      if (!isDirectiveContextAllowed(text, index)) {
        return null;
      }
      const targetRaw = (
...
export function parseSwarmMessage(text: string): ParsedSwarmMessage {
  ...
  while (cursor < text.length) {
    ...
    const unboundedBodyEnd = nextControl?.index ?? text.length;
    const rawBodyCandidate = text.slice(bodyStart, unboundedBodyEnd);
    const paragraphBoundary = findParagraphBoundary(rawBodyCandidate);
    const bodyEnd = paragraphBoundary === null ? unboundedBodyEnd : bodyStart + paragraphBoundary;
