...
const UPLOADED_SKILL_CONTENT_LIMIT = 48_000;

type SkillUploadFile = File & { webkitRelativePath?: string };

function formatSkillUploadContent(files: SkillUploadFile[]): Promise<string> {
  return Promise.all(
    ...
    return content.length > UPLOADED_SKILL_CONTENT_LIMIT
      ? `${content.slice(0, UPLOADED_SKILL_CONTENT_LIMIT)}\n\n[Skill upload truncated for prompt safety.]`
      : content;
  });
}

async function buildUploadedSkillsFromFiles(
  fileList: FileList,
): Promise<SwarmUploadedSkillDraft[]> {
  const files = Array.from(fileList) as SkillUploadFile[];
  ...
  if (hasFolderPaths) {
    const folderName = files[0]?.webkitRelativePath?.split("/")[0] || "Uploaded skill folder";
    const skillMdFile = files.find((file) => file.webkitRelativePath?.endsWith("/SKILL.md"));
    const sortedFiles = skillMdFile
      ? [skillMdFile, ...files.filter((file) => file !== skillMdFile)]
      : files;
    return [
      {
        id: randomUUID(),
        ...
        path: folderName,
        sourceType: "folder",
        content: await formatSkillUploadContent(sortedFiles),
  ...
  return Promise.all(
    files.map(async (file) => ({
      id: randomUUID(),
      name: file.name.replace(/\/?SKILL\.md$/i, "") || file.name,
      path: file.name,
      sourceType: "file" as const,
      content: await formatSkillUploadContent([file]),
      fileCount: 1,
      uploadedAt,
    })),
  );
...
export function SwarmWizard({ projectId, onCreate, busy = false }: SwarmWizardProps) {
  ...
  const [isReadingSkillUpload, setIsReadingSkillUpload] = useState(false);
  ...
  const defaultProvider: ProviderKind =
    availableProviders[0] ?? ProviderDriverKind.make("opencode");
  ...
  const [bulkProviderInstanceId, setBulkProviderInstanceId] = useState<string | undefined>();
  ...
  const handleSkillFilesSelected = async (fileList: FileList | null) => {
    if (!fileList) return;
    setIsReadingSkillUpload(true);
    try {
      const nextSkills = await buildUploadedSkillsFromFiles(fileList);
      addUploadedSkills(nextSkills);
    } finally {
      setIsReadingSkillUpload(false);
    }
  };
  ...
  const handleAddSkillPath = () => {
    ...
    if (!path) return;
    const name = path.split("/").filter(Boolean).at(-2) ?? path.split("/").pop() ?? path;
    addUploadedSkills([
      {
        id: randomUUID(),
        name: name.replace(/\/?SKILL\.md$/i, "") || name,
        path,
        sourceType: path.endsWith("/SKILL.md") ? "file" : "folder",
  ...
  const handleCreate = () => {
    ...
    void onCreate({
      ...config,
      enableTasks: (settings as { enableSwarmTasks?: boolean }).enableSwarmTasks ?? false,
    });
  };
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="space-y-3 border-t border-border/60 pt-6">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            <p className="text-sm font-semibold text-foreground">Uploaded Skills</p>
                            <p className="text-xs text-muted-foreground">
                              Persist reusable skill files or folders and toggle them into this
                              swarm. Selected uploads are injected into every agent prompt.
                            </p>
                          </div>
                          <div className="flex shrink-0 flex-wrap gap-2">
                            <Button
                              ...
                              size="sm"
                              className="gap-2"
                              disabled={isReadingSkillUpload}
                              onClick={() => skillFileInputRef.current?.click()}
                            >
                              {isReadingSkillUpload ? (
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row">
                          <Input
                            ...
                            placeholder="/home/dani/.codex/skills/roblox-studio-framework-skill/SKILL.md"
                          ...
                          <Button
                            ...
                            disabled={skillPathInput.trim().length === 0}
                          >
                            Add path
                          </Button>
                        </div>
                        ...
                        <input
                          ...
                          className="hidden"
                          onChange={(e) => {
                            void handleSkillFilesSelected(e.target.files);
                            e.target.value = "";
                          }}
                        />
                        <input
                        ...
                        {uploadedSkills.length === 0 ? (
                          <div className="rounded-2xl border border-dashed border-border/60 bg-background/70 p-4 text-sm text-muted-foreground">
                            No uploaded skills yet. Upload a `SKILL.md`, choose a skill folder, or
                            add an absolute path to make it available across swarm builds.
                          </div>
                        ) : (
                          <div className="grid gap-3 sm:grid-cols-2">
                            {uploadedSkills.map((skill) => {
                              const isSelected = selectedUploadedSkillIds.includes(skill.id);
                              return (
                                <div
                                  key={skill.id}
                                  className={cn(
                                    "group flex items-start gap-3 rounded-2xl border p-4 text-left text-sm transition",
                                    isSelected
                                      ? "border-primary/60 bg-primary/10 text-foreground shadow-lg shadow-primary/10"
                                      : "border-border/50 bg-background hover:border-primary/50 hover:bg-muted/40",
                                  )}
                                >
                                  <button
                                    type="button"
                                    onClick={() => toggleUploadedSkill(skill.id)}
                                    aria-pressed={isSelected}
                                    className="flex min-w-0 flex-1 items-start gap-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
                                  >
                                    <span
                                      className={cn(
                                    ...
                                    <div className="min-w-0 flex-1">
                                      ...
                                      <p className="mt-1 text-xs text-muted-foreground">
                                        {skill.sourceType}
                                        {skill.fileCount ? ` · ${skill.fileCount} file(s)` : ""}
                                      </p>
                                    </div>
                                  </button>
                                  <Button
                                    ...
                                    onClick={() => removeUploadedSkill(skill.id)}
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between animate-in fade-in zoom-in-95">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <ModelPicker
                          ...
                          providerInstanceId={bulkProviderInstanceId}
                          model={bulkModel}
                          onProviderModelChange={(provider, model, providerInstanceId) => {
                            setBulkProvider(provider);
                            setBulkProviderInstanceId(providerInstanceId);
                            setBulkModel(model);
                            applyBulkUpdate({ provider, model, providerInstanceId });
                          }}
                        ...
                        <CardContent className="grid gap-5 p-5 sm:grid-cols-2 lg:grid-cols-6">
                          ...
                          <div className="space-y-1.5 lg:col-span-2">
                            ...
                            <ModelPicker
                              ...
                              onProviderModelChange={(provider, model, providerInstanceId) =>
                                updateAgent(agent.id, { provider, model, providerInstanceId })
                              }
                              className={cn(
                                "w-full",
                                isSelected
