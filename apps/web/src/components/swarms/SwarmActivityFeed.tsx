...
export function SwarmActivityFeed({
  ...
  const messagesByAgent = useMemo(() => {
    const byAgent = new Map<
      string,
      { entry: (typeof messages)[number]; formatted: ReturnType<typeof formatSwarmMessage> }[]
    >();
    for (const entry of messages) {
      const formatted = formatSwarmMessage(
        entry,
        swarm.config.agents,
        showRawDirectives || entry.streaming,
      );
      if (formatted.hideWhenNotRaw) continue;
      const agentKey =
        entry.sender === "operator" ? "operator" : (entry.senderAgentId ?? "unknown");
  ...
  const markdownComponents = {
    ...
    blockquote: ({ children }: any) => (
      <blockquote className="border-l-2 border-foreground/30 pl-3 italic opacity-80 my-2">
        {children}
      </blockquote>
    ),
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 ml-1 border-l border-border/50 pl-2">
            <Button
              ...
              title={
                showRawDirectives ? "Hide raw directive messages" : "Show raw directive messages"
              }
            >
              OG
            </Button>
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <div
                        className={`flex size-6 items-center justify-center rounded-full border bg-background shadow-sm ${
                          isOperator
                    ...
                    <div className="relative space-y-4 px-3 py-3 before:absolute before:inset-y-3 before:left-[11px] before:w-px before:bg-border/50">
                      {items.map(({ entry, formatted }) => {
                        const routeLabel = getSwarmMessageRouteLabel({
                          sender: entry.sender,
                          senderName: isOperator ? "Operator" : headerName,
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`absolute left-0 top-1.5 flex size-[22px] items-center justify-center rounded-full border bg-background shadow-sm ${
                                isOperator
                                  ? "border-primary/50 text-primary"
                                  : "border-border text-muted-foreground"
                              }`}
                            ...
                            <div className="flex flex-col gap-1.5 rounded-lg border border-border/50 bg-background/50 p-3 shadow-sm backdrop-blur-sm transition-colors hover:bg-background">
                              <div className="flex flex-wrap items-center justify-between gap-2">
                                <div className="flex items-center gap-1.5 text-[11px] font-medium tracking-wide text-muted-foreground uppercase">
                                  <span className={isOperator ? undefined : "text-foreground"}>
                                    {routeLabel}
                                  </span>
                                </div>
