...
const makeProjectionThreadSwarmRepository = Effect.gen(function* () {
  ...
  `.pipe(
    Effect.mapError(
      toPersistenceSqlError("ProjectionThreadSwarmRepository.ensureSchema:createTable"),
    ),
  );
  ...
  `.pipe(
    Effect.mapError(toPersistenceSqlError("ProjectionThreadSwarmRepository.ensureSchema:index")),
  );
